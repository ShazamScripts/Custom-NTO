...
TargetBot.Looting.getLootContainers = function(containers)
  ...
  local toOpen = nil
  for index, container in pairs(containers) do
    openedContainersById[(container:getContainerItem() and container:getContainerItem():getId())] = 1
    if containersById[(container:getContainerItem() and container:getContainerItem():getId())] and not container.lootContainer then
  ...
  if not lootContainers[1] then
    ...
    for index, container in pairs(containers) do
      if not containersById[(container:getContainerItem() and container:getContainerItem():getId())] and not container.lootContainer then
        for slot, item in ipairs(container:getItems()) do
          if item:isContainer() and containersById[item:getId()] then
            g_game.open(item)
            waitTill = now + 500 -- wait 0.5s
...
onContainerOpen(function(container, previousContainer)
  if (container:getContainerItem() and container:getContainerItem():getId()) == waitingForContainer then
