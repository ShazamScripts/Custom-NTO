    ...
    -- player
      ...
      if tile:hasCreature() then
        ...
        if c and c:isPlayer() then
          ...
          local candidates = {}
          -- FIX (Slow macro): so precisamos das tiles a distancia 1 da
          -- criatura (vizinhas), nao de varrer todas as tiles visiveis.
          ...
          for _, offset in ipairs(neighborOffsets) do
            local nPos = {x = cPos.x + offset[1], y = cPos.y + offset[2], z = cPos.z}
            local isPlayerTile = (nPos.x == pPos.x and nPos.y == pPos.y and nPos.z == pPos.z)
            if not isPlayerTile then
              local nTile = g_map.getTile(nPos)
              if nTile and nTile:isWalkable() then
