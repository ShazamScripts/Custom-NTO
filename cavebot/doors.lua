...
CaveBot.Extensions.OpenDoors.setup = function()
  CaveBot.registerAction("OpenDoors", "#00FFFF", function(value, retries)
    ...
    if not doorTile then
      -- FIX (Slow macro): ja sabemos a posicao exata da porta (pos), nao
      -- precisa varrer todas as tiles visiveis pra encontrar ela -- g_map.getTile
      -- ja busca direto pela posicao.
      doorTile = g_map.getTile(pos)
    end
